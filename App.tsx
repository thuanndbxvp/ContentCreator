...
import { generateScript, generateScriptOutline, generateTopicSuggestions, reviseScript, generateScriptPart, extractDialogue, generateKeywordSuggestions, validateApiKey, generateVisualPrompt, generateAllVisualPrompts } from './services/geminiService';
import type { StyleOptions, FormattingOptions, LibraryItem, GenerationParams, VisualPrompt, AllVisualPromptsResult } from './types';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    try {
      ...
      if (savedApiKey) {
        ...
        // Automatically open the API key modal if no key is found on first load
  ...
  const handleValidateAndSaveApiKey = async (key: string) => {
  ...
  const handleGenerateScript = useCallback(async () => {
    ...
    try {
      const isLongScript = parseInt(wordCount, 10) > 1000;
      if (isLongScript) {
        const outline = await generateScriptOutline(topic, wordCount, targetAudience);
        setGeneratedScript(outline);
      } else {
        const script = await generateScript({ topic, targetAudience, styleOptions, keywords, formattingOptions, wordCount, scriptParts });
  ...
  }, [topic, targetAudience, styleOptions, keywords, formattingOptions, wordCount, scriptParts]);
  ...
  const handleReviseScript = useCallback(async () => {
    ...
    const params: GenerationParams = { topic, targetAudience, styleOptions, keywords, formattingOptions, wordCount, scriptParts };
  ...
  }, [revisionPrompt, generatedScript, topic, targetAudience, styleOptions, keywords, formattingOptions, wordCount, scriptParts]);
  ...
  const handleGenerateNextPart = useCallback(async () => {
      ...
      try {
          ...
          const params = { targetAudience, styleOptions, keywords, formattingOptions, wordCount, scriptParts };
  ...
  }, [currentPartIndex, outlineParts, isGeneratingSequentially, generatedScript, targetAudience, styleOptions, keywords, formattingOptions, wordCount, scriptParts]);
  ...
  return (
    <div className="min-h-screen bg-primary font-sans">
      <header className="bg-secondary/50 border-b border-secondary p-4 shadow-lg flex justify-between items-center">
        ...
        <div className="flex-1 flex justify-end items-center gap-4">
            ...
            <button 
                ...
                className="flex items-center gap-2 p-2 bg-secondary hover:bg-primary text-text-primary font-semibold rounded-lg transition-colors"
      ...
      <main className="flex flex-col md:flex-row gap-6 p-4 md:p-6 max-w-7xl mx-auto">
        <div className="w-full md:w-2/5 lg:w-1/3 flex-shrink-0">
          <ControlPanel
            ...
            isLoading={isLoading || isSuggesting || isSuggestingKeywords || !apiKey}
      ...
      <ApiKeyModal
        ...
        onSave={handleValidateAndSaveApiKey}
